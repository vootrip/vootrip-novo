          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="text-3xl lg:text-4xl font-bold text-white mb-4">
                Pacotes <span className="text-[#FF6B35]">Completos</span>
              </h2>
            ...
            <div className="grid md:grid-cols-3 gap-6">
              {packagesLoading ? (
                <div className="col-span-3 text-center py-12">
                  <Loader2 className="w-8 h-8 animate-spin text-[#FF6B35] mx-auto" />
                  <p className="text-white/70 mt-2">Carregando pacotes...</p>
                </div>
              ) : (
                pacotes.map((pacote) => (
                  <div key={pacote.id} className={`relative bg-white rounded-2xl p-6 ${pacote.destaque ? 'ring-2 ring-[#FF6B35] scale-105' : ''}`}>
                    {pacote.destaque && (
                      <div className="absolute -top-3 left-1/2 -translate-x-1/2 px-4 py-1 bg-[#FF6B35] text-white text-sm font-bold rounded-full">
                        Mais vendido
                      </div>
                    )}
                    <h3 className="text-xl font-bold text-[#1A1A2E] mb-2">{pacote.nome}</h3>
                    ...
                    <p className="text-2xl font-bold text-[#FF6B35] mb-4">{pacote.preco}</p>
                    <button
                      onClick={() => setModalOpen(true)}
