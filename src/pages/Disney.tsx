        ...
        <section data-ev-id="ev_e45a7108bf" className="py-20 bg-white">
          <div data-ev-id="ev_38bd47226d" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div data-ev-id="ev_dd6b0d4d3d" className="grid md:grid-cols-2 gap-6">
              ...
              <div data-ev-id="ev_409f18fbea" key={index} className="group relative rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all">
                  <div data-ev-id="ev_af342fc627" className="aspect-video overflow-hidden">
                    <img data-ev-id="ev_c1004ce9db" src={parque.imagem} alt={parque.nome} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  </div>
                  <div data-ev-id="ev_340ae6a3e9" className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                  <div data-ev-id="ev_31c4bc86ce" className="absolute bottom-0 left-0 right-0 p-6">
                    <h3 data-ev-id="ev_639b76b2bb" className="text-2xl font-bold text-white mb-2">{parque.nome}</h3>
                    <p data-ev-id="ev_f66bcba655" className="text-white/80 mb-3">{parque.descricao}</p>
                    <div data-ev-id="ev_bfd6dbb7bf" className="flex flex-wrap gap-2">
                      {parque.destaques.map((destaque, i) =>
                    <span data-ev-id="ev_07d0a6e0d9" key={i} className="px-3 py-1 bg-white/20 backdrop-blur-sm text-white text-xs rounded-full">
                          {destaque}
                        </span>
                    )}
                    </div>
        ...
        <section data-ev-id="ev_453b1ec06f" className="py-20 bg-gradient-to-br from-[#1A1A2E] to-[#2D2D44]">
          <div data-ev-id="ev_a2ff2cf706" className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div data-ev-id="ev_66aecf84b8" className="grid md:grid-cols-3 gap-6">
              ...
              <div data-ev-id="ev_66beb55819" key={index} className={`relative bg-white rounded-2xl p-6 ${pacote.destaque ? 'ring-2 ring-[#FF6B35] scale-105' : ''}`}>
                  {pacote.destaque &&
